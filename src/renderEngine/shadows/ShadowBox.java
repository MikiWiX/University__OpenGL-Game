 ...
 * renderEngine.shadows (basically represents the orthographic projection area for the shadow
 ...
 * being too small to avoid objects not having renderEngine.shadows when they should.
...
public class ShadowBox {
	...
	private static final float OFFSET = Settings.SHADOW_OFFSET;
	private static final Vector4f UP = new Vector4f(0, 1, 0, 0);
	private static final Vector4f FORWARD = new Vector4f(0, 0, -1, 0);
	private static final float SHADOW_DISTANCE = Settings.SHADOW_DISTANCE;
	 ...
	 * (within a certain range) will cast renderEngine.shadows.
	 */
	...
	private Matrix4f calculateCameraRotationMatrix() {
		...
		rotation.rotate((float) Math.toRadians(-cam.getYawDY()), new Vector3f(0, 1, 0));
		rotation.rotate((float) Math.toRadians(-cam.getPitchDX()), new Vector3f(1, 0, 0));
		return rotation;
	}
	 ...
	 * but means that distant objects wouldn't cast renderEngine.shadows.
	 */
	...
	private float getAspectRatio() {
		return (float) GameMain.getWidth() / (float) GameMain.getHeight();
	}
